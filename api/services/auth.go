...
func (s *service) AuthUser(ctx context.Context, req *requests.UserAuth, sourceIP string) (*models.UserAuthResponse, int64, error) {
	...
	if s.cfg.MaximumLoginTimeout > 0 {
		if timeout, _ := s.isBlocked(ctx, sourceIP, user.ID); timeout > 0 {
			return nil, timeout, nil
		}
	}

	if !user.Password.Compare(req.Password) {
		timeout, _ := s.storeAttempt(ctx, sourceIP, user.ID)

		return nil, timeout, NewErrAuthUnathorized(nil)
	}

	// Reset the attempt and timeout values when succeeds
	if s.cfg.MaximumLoginTimeout > 0 {
		if err := s.resetAttempts(ctx, sourceIP, user.ID); err != nil {
			log.WithError(err).
